        ...
        android.os.Handler(android.os.Looper.getMainLooper()).postDelayed({
            coroutineScope.launch(Dispatchers.IO) {
                try {
                    ...
                    cachedApps = appManager.getInstalledApps(context, true)
                    selectLetter("A")
...
fun getLetterPositions(): Map<String, Int> {
    ...
    for ((index, app) in apps.withIndex()) {
        ...
        for (letter in 'A'..'Z') {
            ...
            val nextLetter = (letter + 1).toString()
            if (collator.compare(name, letterStr) >= 0 && collator.compare(name, nextLetter) < 0) {
