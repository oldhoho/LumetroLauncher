...
class SidebarManager(private val context: Context) {
    ...
    val lm = SpannedGridLayoutManager(RecyclerView.VERTICAL, 8, 4).apply {
        spanSizeLookup = SpannedGridLayoutManager.SpanSizeLookup { p -> when (cachedTiles.getOrNull(p)?.tileSize) { 1 -> SpanSize(2, 2); 2 -> SpanSize(4, 2); else -> SpanSize(1, 1) } }
    }
...
inner class TileAdapter(private var tiles: List<TileEntity>, private val iconSize: Int) : RecyclerView.Adapter<TileAdapter.VH>() {
    ...
    val sm = when (t.tileSize) { 1 -> 1.5f; 2 -> 2f; 3 -> 0.8f; else -> 1f }; val ss = (iconSize * sm).toInt()
    ...
    val doFlip = object : Runnable {
        override fun run() {
            ...
            anim.start()
            h.c.postDelayed(this, 20000 + (Math.random() * 10000).toLong())
        }
    }
    h.c.postDelayed(doFlip, (Math.random() * 15000).toLong())
}
...
private fun createEditPanelView(t: TileEntity): View {
    ...
    il.addView(ImageView(context).apply { layoutParams = LinearLayout.LayoutParams(when (t.tileSize) { 1 -> 90.dpToPx(); 2 -> 120.dpToPx(); 3 -> 48.dpToPx(); 4 -> 36.dpToPx(); else -> 60.dpToPx() }, when (t.tileSize) { 1 -> 90.dpToPx(); 2 -> 120.dpToPx(); 3 -> 48.dpToPx(); 4 -> 36.dpToPx(); else -> 60.dpToPx() }); scaleType = ImageView.ScaleType.FIT_CENTER; t.tilePackage?.let { pkg -> coroutineScope.launch(Dispatchers.IO) { iconLoader.getIconForPackage(context, pkg)?.let { withContext(Dispatchers.Main) { setImageBitmap(it) } } } } })
