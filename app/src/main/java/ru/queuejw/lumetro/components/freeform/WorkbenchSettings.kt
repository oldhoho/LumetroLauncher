...
class WorkbenchSettings(context: Context) {
    ...
    var height: Int
        ...
        set(value) {
            prefs.edit().putInt("workbench_height", value).apply()
        }

    var enabled: Boolean
        get() = prefs.getBoolean("workbench_enabled", true)
        set(value) {
            prefs.edit().putBoolean("workbench_enabled", value).apply()
        }

    var showAppsListOnStart: Boolean
        get() = prefs.getBoolean("workbench_show_apps_on_start", false)
        set(value) {
            prefs.edit().putBoolean("workbench_show_apps_on_start", value).apply()
        }

    var autoHideDelay: Int
        get() = prefs.getInt("workbench_auto_hide_delay", 0)
        set(value) {
            prefs.edit().putInt("workbench_auto_hide_delay", value).apply()
        }
    ...
    var appListWidthRatio: Float
        ...
        set(value) {
            prefs.edit().putFloat("app_list_width_ratio", value.coerceIn(0.5f, 1.0f)).apply()
        }

    var appListHeightRatio: Float
        get() = prefs.getFloat("app_list_height_ratio", 0.9f)
        set(value) {
            prefs.edit().putFloat("app_list_height_ratio", value.coerceIn(0.1f, 2.0f)).apply()
        }

    var appListVerticalOffset: Int
        get() = prefs.getInt("app_list_vertical_offset", 0)
        set(value) {
            prefs.edit().putInt("app_list_vertical_offset", value.coerceIn(0, 400)).apply()
        }

    var appListCornerRadius: Int
        get() = prefs.getInt("app_list_corner_radius", 24)
        set(value) {
            prefs.edit().putInt("app_list_corner_radius", value.coerceIn(0, 50)).apply()
        }

    var appListDimAlpha: Float
        get() = prefs.getFloat("app_list_dim_alpha", 0.8f)
        set(value) {
            prefs.edit().putFloat("app_list_dim_alpha", value.coerceIn(0.0f, 0.9f)).apply()
        }
    ...
    var tilesPanelEnabled: Boolean
        ...
        set(value) {
            prefs.edit().putBoolean("tiles_panel_enabled", value).apply()
        }
